terraform {
  required_providers {
    azurerm = {
      ...
      version               = "=3.36.0"
      configuration_aliases = [azurerm.spoke1-sub, azurerm.identity-sub]
    }
  }
  backend "azurerm" {
  }
...
provider "azurerm" {
  ...
  alias           = "identity-sub"
  subscription_id = var.azure_subscription_ids["identity-sub"]
  tenant_id       = var.azure_tenant_id
  client_secret   = var.azure_client_secret
  client_id       = var.azure_client_id
}
...
data "terraform_remote_state" "monitor_resources" {
  backend = "azurerm"
  config = {
    container_name       = var.state_storage_container_name
    storage_account_name = var.state_storage_acct_name
...
data "terraform_remote_state" "resource_group_resources" {
  backend = "azurerm"
  config = {
    container_name       = var.state_storage_container_name
    storage_account_name = var.state_storage_acct_name
    access_key           = var.tfstate_storage_access_key
    key                  = "eslzspoke/${var.clientName}/${var.environment}/core/project.tfstate"
  }
}